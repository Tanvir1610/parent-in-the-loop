...
interface SearchResult {
  id: number; title: string; category: string; slug: string; substack_url: string; excerpt: string
}

const CATEGORY_COLORS: Record<string, string> = {
  "AI Literacy": "#7C63B8", Safety: "#3a6e8a", "Family Conversations": "#4d7a49", Parenting: "#c97a5a",
}
  ...
  return (
    <>
      <header
        ...
        style={{
          backgroundColor: scrolled ? "rgba(var(--background-rgb,250,246,240),0.95)" : "rgba(var(--background-rgb,250,246,240),0.8)",
          backdropFilter: "blur(12px)",
          borderBottom: scrolled ? "1px solid rgba(183,157,132,0.2)" : "1px solid transparent",
          boxShadow: scrolled ? "0 2px 20px rgba(0,0,0,0.06)" : "none",
          background: scrolled
            ? "var(--header-bg-scrolled, rgba(250,246,240,0.95))"
            : "var(--header-bg, rgba(250,246,240,0.8))",
        }}
      >
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden md:flex items-center gap-1" aria-label="Main navigation">
              {NAV_LINKS.map(link => (
                <button key={link.id} onClick={() => scrollTo(link.id)}
                  ...
                  style={{ color: "var(--foreground, #3E3E3E)", fontFamily: "var(--font-nunito), Nunito, sans-serif" }}
                  onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.color = "#7C63B8" }}
                  onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.color = "var(--foreground, #3E3E3E)" }}>
                  {link.label}
                </button>
              ))}
            </nav>
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setSearchOpen(!searchOpen)}
                ...
                style={{ color: "#B79D84" }}
                onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.color = "#7C63B8" }}
                onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.color = "#B79D84" }}
            ...
            <div className="pb-3 relative">
              <div className="relative">
                <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none"
                  style={{ color: "#B79D84" }} aria-hidden="true" />
                <input ref={searchRef} type="search" value={query} onChange={e => setQuery(e.target.value)}
                  ...
                  style={{ backgroundColor: "var(--card, #fff)", borderColor: "#EDE8E1", color: "var(--foreground, #222222)", fontFamily: "var(--font-nunito), Nunito, sans-serif" }}
                ...
                {searching && (
                  <div className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 rounded-full border-2 border-t-transparent animate-spin"
                    style={{ borderColor: "#7C63B8", borderTopColor: "transparent" }} aria-hidden="true" />
                )}
              </div>
              {results.length > 0 && (
                <div className="absolute left-0 right-0 top-full mt-1 rounded-xl overflow-hidden shadow-xl z-50"
                  style={{ backgroundColor: "var(--card, #fff)", border: "1px solid #EDE8E1" }}
                  role="listbox" aria-label="Search results">
                  {results.map(r => (
                    <a key={r.id} href={r.substack_url || `/articles/${r.slug}`}
                      target={r.substack_url ? "_blank" : "_self"}
                      rel={r.substack_url ? "noopener noreferrer" : ""}
                      className="flex items-start gap-3 px-4 py-3 hover:bg-[#FAF6F0] dark:hover:bg-[#1A1A2E] transition-colors focus:outline-none"
                      onClick={() => setSearchOpen(false)} role="option">
                      <span className="mt-0.5 px-2 py-0.5 rounded-full text-xs font-bold flex-shrink-0"
                        style={{ color: CATEGORY_COLORS[r.category] ?? "#7C63B8", backgroundColor: `${CATEGORY_COLORS[r.category] ?? "#7C63B8"}18` }}>
                        {r.category}
                      </span>
                      <div>
                        <p className="text-sm font-bold line-clamp-1" style={{ color: "var(--foreground, #222222)", fontFamily: "var(--font-quicksand), Quicksand, sans-serif" }}>{r.title}</p>
                        <p className="text-xs line-clamp-1 mt-0.5" style={{ color: "#B79D84", fontFamily: "var(--font-nunito), Nunito, sans-serif" }}>{r.excerpt}</p>
                      </div>
                    </a>
                  ))}
                </div>
              ...
              {query.trim() && !searching && results.length === 0 && (
                <div className="absolute left-0 right-0 top-full mt-1 rounded-xl px-4 py-3 text-sm shadow-xl z-50"
                  style={{ backgroundColor: "var(--card, #fff)", border: "1px solid #EDE8E1", color: "#B79D84", fontFamily: "var(--font-nunito), Nunito, sans-serif" }}>
        ...
        {menuOpen && (
          <div className="md:hidden border-t px-4 py-4 space-y-1"
            style={{ backgroundColor: "var(--background, #FAF6F0)", borderColor: "rgba(183,157,132,0.2)" }}>
            {NAV_LINKS.map(link => (
              <button key={link.id} onClick={() => scrollTo(link.id)}
    ...
    <>
      {isSignedIn ? (
        <div className="hidden md:flex items-center gap-3">
          ...
          <UserButton appearance={{ elements: { avatarBox: "w-9 h-9 rounded-xl border-2", userButtonPopoverCard: "rounded-2xl shadow-xl" } }} afterSignOutUrl="/" />
        </div>
      ) : (
        <div className="hidden md:flex items-center gap-2">
          ...
          <Link href="/sign-up"
            className="px-4 py-2 rounded-xl text-sm font-bold text-white transition-all hover:scale-105 active:scale-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#F3A78E]"
...
function MobileAuthButton({ setMenuOpen }: { setMenuOpen: (v: boolean) => void }) {
  ...
  return isSignedIn ? (
    <Link href="/dashboard" onClick={() => setMenuOpen(false)}
      className="block w-full text-center px-4 py-3 rounded-xl font-semibold text-sm border-2"
