...
const AGE_LABELS: Record<string, string> = {
  "5-7": "Ages 5–7", "8-10": "Ages 8–10", "11-13": "Ages 11–13",
  "14-16": "Ages 14–16", "17+": "Ages 17+",
}
...
export default function ArticleCard({ article }: { article: Article }) {
  ...
  const external = !!article.substack_url
  const ageLabel = article.age_group && article.age_group !== "all" ? AGE_LABELS[article.age_group] : null

  return (
    <article
      className="group h-full flex flex-col rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1 focus-within:ring-2 focus-within:ring-[#7C63B8]"
    ...
    >
      ...
      <Link href={href} target={external ? "_blank" : "_self"} rel={external ? "noopener noreferrer" : ""}
        className="block overflow-hidden flex-shrink-0 focus:outline-none" tabIndex={-1} aria-hidden="true">
        <div className="relative h-48 overflow-hidden" style={{ backgroundColor: "#EDE8E1" }}>
          ...
          {ageLabel && (
            <span className="absolute top-3 left-3 px-2 py-0.5 rounded-full text-[10px] font-bold"
              style={{ backgroundColor: "rgba(166,182,161,0.95)", color: "#2D5016" }}>
              {ageLabel}
            </span>
          )}
          {/* Featured badge */}
          {article.featured && (
            <span className="absolute top-3 right-3 px-2 py-0.5 rounded-full text-[10px] font-bold"
              style={{ backgroundColor: "#F3A78E", color: "#fff" }}>
              ✦ Featured
            </span>
          )}
      ...
      <div className="p-5 flex flex-col flex-1 gap-3">
        ...
        <h3 className="text-lg font-bold leading-snug"
          ...
          <Link href={href} target={external ? "_blank" : "_self"} rel={external ? "noopener noreferrer" : ""}
            ...
            style={{ color: "var(--foreground, #222222)" }}
            onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.color = "#7C63B8" }}
            onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.color = "var(--foreground, #222222)" }}
            aria-label={`Read "${article.title}"${external ? " on Substack" : ""}`}>
            {article.title}
          </Link>
        </h3>
          ...
          <div className="flex flex-wrap gap-1.5">
            {article.tags.slice(0, 3).map(tag => (
              <span key={tag} className="text-xs px-2 py-0.5 rounded-full"
                style={{ backgroundColor: "var(--muted, #EDE8E1)", color: "#B79D84", fontFamily: "var(--font-nunito), Nunito, sans-serif" }}>
                #{tag}
              </span>
            ))}
          </div>
        ...
        <div className="pt-1 mt-auto">
          <Link href={href} target={external ? "_blank" : "_self"} rel={external ? "noopener noreferrer" : ""}
            ...
            {external ? "Read on Substack" : "Read article"} <span aria-hidden="true">→</span>
          </Link>
        </div>
      </div>
    </article>
